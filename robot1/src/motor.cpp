...
{ 
  ...
  ros::NodeHandle nh;
//  ros::Publisher robot_sensor_pub = nh.advertise<geometry_msgs::Twist>("sensor_data",1000);
  ros::Subscriber sub = nh.subscribe("motor_reference1", 1, motor_callback1);
  ros::Subscriber sub2 = nh.subscribe("motor_reference2", 1, motor_callback2);
  //ros::Timer timer3 = nh.createTimer(ros::Duration(0.01),increase_time_3);
   ...
   while(ros::ok()) 
  {
     ...
     //motor_control(1, motor1_IN1, motor1_IN2, 1, 150, 0, 1); 
     //motor_control(2, motor2_IN3, motor2_IN4, 0, 150, 0, 1); 
   //  motor_control2(motor2_PWM_1, 0, 20, 0, 1);  

//printf("motor 1 rpm : %d \n", result_rpm);
//printf("motor 2 rpm : %f \n", result_rpm2);
